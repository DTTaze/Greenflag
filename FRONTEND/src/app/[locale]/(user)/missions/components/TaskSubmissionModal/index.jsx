      ...
      <div className="w-full max-w-lg overflow-hidden rounded-2xl bg-white shadow-2xl border border-gray-100 flex flex-col max-h-[90vh]">
        ...
        <div className="flex-1 overflow-y-auto p-6 space-y-6">
          ...
          <div className="space-y-3">
            ...
            <div className="relative border-2 border-dashed border-gray-200 rounded-2xl p-6 bg-gray-50/20 hover:bg-gray-50/80 transition-colors flex flex-col items-center justify-center gap-2 group cursor-pointer">
              <input
                type="file"
                accept="image/*"
                multiple
              ...
              />
              <div className="flex h-11 w-11 items-center justify-center rounded-xl bg-emerald-50 text-[#0B6E4F] group-hover:scale-105 transition-transform duration-300">
                <UploadCloud className="h-5.5 w-5.5" />
              </div>
              <span className="text-xs font-bold text-gray-600">Nhấp hoặc kéo thả ảnh vào đây</span>
              <span className="text-[10px] font-semibold text-gray-400">Hỗ trợ định dạng JPG, PNG, WEBP</span>
            </div>
              ...
              <div className="rounded-xl bg-emerald-50/30 border border-emerald-100 p-4.5 space-y-2">
                ...
                <div className="text-[11px] font-semibold text-emerald-800/80">
                  ...
                  <strong className="text-xs text-[#0B6E4F]">
                    +{Math.min(files.length, task.total - (task.progress_count || 0))}
                  </strong>
                </div>
                {/* List names of files */}
                <div className="max-h-[80px] overflow-y-auto pt-1 border-t border-emerald-100/30 space-y-1">
                  {files.map((file, i) => (

...
export class CommerceProcessor extends WorkerHost {
  ...
  public async process(job: Job<any, any, string>): Promise<any> {
    ...
    // Extract trace context propagated from publisher
    const traceContext = job.data.traceContext || {};
    const parentContext = propagation.extract(ROOT_CONTEXT, traceContext);
    const tracer = trace.getTracer('commerce-worker');

    return context.with(parentContext, async () => {
      return tracer.startActiveSpan('process-item-purchase', async (span) => {
        ...
        try {
          ...
          const sellerDeliveryAccount =
            await this.deliveryAccountRepository.findOne({
          ...
          // Record metrics and trace status
          this.metricsService.incrementSuccessfulPurchases(1, {
          ...
          // Record metrics and trace error
          this.metricsService.incrementFailedPurchases(1, {
